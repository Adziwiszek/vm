...
bool tableInsert(Table* table, ObjString* key, Value value) {
...
ObjString* tableFindString(Table* table, const char* str, int length, uint32_t hash) {
  ...
  for(;;) {
    ...
    if(!entry->key) {
      ...
      // if hash matches we must check actual strings too
      return entry->key;
    }
